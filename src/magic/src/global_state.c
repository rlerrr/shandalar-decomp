...
unsigned int save_or_load_ver1(void)
{
  ...
  result &= save_or_load_data(&duel_summary, 0x60);
  ...
  result &= save_or_load_data(creature_power_by_color, 0x40);
  result &= save_or_load_data(creature_toughness_by_color, 0x40);
  result &= save_or_load_data(ai_mana_demand_by_color, 0x40);
  result &= save_or_load_data(&card_types_in_play, 8);
  result &= save_or_load_data(&creature_count_summary, 8);
  ...
  result &= save_or_load_data(graveyard_card_types, 8);
