...
int put_card_on_stack(int player, int card, int mode)
{
  ...
  if (mode == 0)
  {
    ...
    if (unk_0091bfb4 == 0)
    {
      if ((player == nonactive_player || (g_duel_network_flags & 2) != 0) && g_duel_ai_mode_state != 1)
      {
        ...
        {
          mana_charge[6] = (int)(char)s.card_data->cc[0] + (int)(char)s.card_data->cc[1];
        }
        else
        {
    ...
    if (s.internal_card_id != -1)
    {
      ...
      else
      {
        ...
        if (spell_fizzled != 1)
        {
          if (player == active_player && (g_duel_network_flags & 2) == 0 && g_duel_ai_mode_state == 1 && *(char *)((char *)s.card_data + 0x2c) == -1)
          {
  ...
  else
  {
    ...
    land_can_be_played &= ~0x20;
    PLAYER_CARD_INSTANCE(player, card).state |= (player == 0) ? 0x400080 : 0x80;
    ...
    if (spell_fizzled != 1)
    {
      if (player == active_player && g_duel_ai_mode_state != 1 && DAT_007abc80 == 0 && (((unsigned char)s.card_data->type & 0x7e) != 0))
      {
...
int resolve_card_on_stack(int player, int card)
{
  ...
  {
    sprintf(unk_00748770, gs_cast_008b4720, FUN_0044a3bf(player, card));
...
int graveyard_has_type(int player, int type)
{
  ...
  s.result = 0;
  if (player == -1 || player == nonactive_player)
  {
    for (s.graveyard_index = 0, s.done = 0; s.graveyard_index < 500 && s.done == 0; ++s.graveyard_index)
    {
      if (global_graveyard_slots[nonactive_player][s.graveyard_index] != -1)
      {
        if ((type & global_cards_data[global_graveyard_slots[nonactive_player][s.graveyard_index]].type) != 0)
        {
          s.result = 1;
          s.done = s.result;
        }
  ...
  if ((player == -1 || player == active_player) && s.result == 0)
  {
    for (s.graveyard_index = 0, s.done = 0; s.graveyard_index < 500 && s.done == 0; ++s.graveyard_index)
    {
                 ...
                 int *title)
{
  ...
  if (nonactive_player == player && g_duel_ai_mode_state != 1)
  {
...
void shuffle_duel_library(int player, int deck_owner)
{
  ...
  if ((g_duel_network_flags & 2) == 0 || nonactive_player == player)
  {
    s.deck_size = 500;
...
int show_deck(int player, int *cards, int count, void *context, int suppress_done_txt, char *prompt)
{
  ...
  if (((active_player == player) && ((g_duel_network_flags & 2) != 0)) && unk_00742fc4 != 0)
  {
    TENTATIVE_wait_for_network_result(player, 0x19);
    s.result = g_network_result_value;
  }
  else if (((active_player == player) && ((g_duel_network_flags & 2) == 0)) || g_duel_ai_mode_state == 1)
  {
    s.valid_count = 0;
...
int draw_card_for_player(int player)
{
  ...
  if (player == nonactive_player)
  {
    ...
    {
      life[nonactive_player] = -99;
      life[1 - nonactive_player] = 0x14;
    }
  }
  else
  {
    ...
    if (s.drawn_card == -1)
    {
      ...
      {
        life[1 - nonactive_player] = -99;
        life[nonactive_player] = 0x14;
      }
    }
...
int FUN_004b0047(int player, int card)
{
  ...
  if (s.in_play != 0 && current_phase == 0x15 && player == human_player && (s.state & 0x8000) != 0 && (s.state & 4) == 0 && can_attack(player, card) != 0)
  {
    s.result = 2;
  }
  else if (s.in_play != 0 && trigger_condition != -1)
  {
    ...
    {
      s.trigger_result = ClampIntToRange(FUN_004b082f(player, card, EVENT_TRIGGER, player), 0, 2);
    }
  ...
  else
  {
    ...
    else if (_DAT_00742fbc == 0 && (current_phase == 0x15 || current_phase == 0x17))
    {
      ...
      {
        if (player == human_player && can_attack(player, card) != 0 && (s.state & 0x10000) == 0)
        {
          unk_00743038 = 0;
          return 0x10;
        }
        if (player != human_player && attacking_creature_count != 0 && (s.state & 8) == 0)
        {
          unk_00743038 = 0;
          return 0x20;
        }
    ...
    else
    {
      ...
      else
      {
        ...
        if ((s.state & 0x10) == 0 && (s.type & TYPE_CREATURE) != 0 && _DAT_00742fbc == 0 && player == human_player && current_phase < 0x1b && can_attack(player, card) != 0 && (((s.instance->token_status & 3) == 0) || (global_cards_data[s.internal_card_id].type & TYPE_CREATURE) == 0))
        {
          _DAT_00743024 = 1;
        }

        if ((((s.extra_ability & 0x1000) != 0 && (s.state & 0x10) == 0 && (((s.instance->token_status & 3) == 0) || (global_cards_data[s.internal_card_id].type & TYPE_CREATURE) == 0)) || ((s.extra_ability & 1) != 0 && (DAT_00742f68 & 0x10) != 0) || ((s.extra_ability & 2) != 0 && (DAT_00742f68 & 0x20) != 0)) && ((land_can_be_played & 4) == 0 || (s.extra_ability & 0x5004) != 0))
        {
          unk_008b3270 &= ~2;
...
void FUN_004e4f11(void)
{
  ...
  {
    unk_006a21b8[recorded_action_count] = unk_00925bb8;
    unk_006a8258[recorded_action_count] = PLAYER_CARD_INSTANCE((unk_00925bb8 & 0x100) >> 8, unk_00925bb8 & 0xff).internal_card_id;
    unk_006a8660[recorded_action_count] = unk_0057aae8;
    unk_006a6ee8[recorded_action_count] = unk_00939340;
...
{
  if (unk_006a5f18[recorded_action_count] != unk_0057aae8)
  {
    unk_0057aae8 |= 0x100;
  }

  unk_00925bb8 = unk_006a1db8[recorded_action_count];
  unk_00939340 = recorded_action_codes[recorded_action_count];
  if (unk_00939340 != 99)
  {
    ++recorded_action_count;
  }
...
void discard(int player, int flags, int player_who_controls_effect)
{
  ...
  if ((active_player == player && (g_duel_network_flags & 2) == 0 && (hand_count[player] + unk_007161d8) <= 0))
    return;

  if (((nonactive_player == player || (g_duel_network_flags & 2) != 0) && g_duel_ai_mode_state != 1) && flags == 0)
  {
...

  {
    if (mana_charge[MANA_CHARGE_POWER_ARTIFACT_REDUCTION] > 0)
    {
      if (color == COLOR_COLORLESS)
      {
        if (amount != -1)
        {
          s.result = amount - mana_charge[MANA_CHARGE_POWER_ARTIFACT_REDUCTION] * 2;
          if (s.result <= 1)
          {
            s.result = 1;
          }
      ...
      {
        s.result = mana_charge[COLOR_COLORLESS] - mana_charge[MANA_CHARGE_POWER_ARTIFACT_REDUCTION] * 2;
        if (s.result <= 1)
        {
          s.result = 1;
        }
        mana_charge[COLOR_COLORLESS] = s.result;
      }
    }
    ...
    if (unk_00715fb0 == 0 && ((player != active_player || (g_duel_network_flags & 2) != 0) && g_duel_ai_mode_state != 1 && g_duel_network_state == 0))
    {
    ...
    if (mana_charge[MANA_CHARGE_POWER_ARTIFACT_REDUCTION] < 1)
    {
      x_value = 0;
    }
    else
    ...
    if (s.include_auto_payment != 0 && FUN_004346ab(mana_charge, x_value, max_x_value) == 0)
    {
      FUN_0043410a(player, s.mana_paid_by_color, &s.mana_paid_total, s.max_colorless);
      copy_mana_pool_to_display();
    }

    if (s.allow_special_x_payment != 0 && FUN_004346ab(mana_charge, x_value, max_x_value) == 0 && s.payable_colors != 0)
    {
      FUN_004342b3(player, s.mana_paid_by_color, &s.mana_paid_total, s.max_colorless, &x_value, max_x_value);
      copy_mana_pool_to_display();
    }

    if (s.allow_generic_payment != 0 && FUN_004346ab(mana_charge, x_value, max_x_value) == 0)
    {
      auto_pay_generic_mana(player, s.mana_paid_by_color, &s.mana_paid_total, s.max_colorless, &x_value, max_x_value);
      copy_mana_pool_to_display();
    }

    if (s.allow_mana_sources != 0 && FUN_004346ab(mana_charge, x_value, max_x_value) == 0)
    {
      if (FUN_004346ab(mana_charge, x_value, max_x_value) == 0)
      {
    ...
    if (s.allow_interrupt_speed_mana_sources != 0 && FUN_004346ab(mana_charge, x_value, max_x_value) == 0)
    {
      if (FUN_004346ab(mana_charge, x_value, max_x_value) == 0)
      {
    ...
    {
      if (FUN_004346ab(mana_charge, x_value, max_x_value) == 0 && s.payable_colors == 0)
      {
        spell_fizzled = 1;
      }
    }
    else if (FUN_004346ab(mana_charge, x_value, max_x_value) == 0)
    {
      s.done = 0;
      while (s.done == 0 && FUN_004346ab(mana_charge, x_value, max_x_value) == 0)
      {
        ...
        FUN_0043479e(unk_00748770, mana_charge, x_value, max_x_value);
        s.selected_card =
          ...
          else if (g_target_selection_status_code == -3 && player == DAT_00715fa4 && DAT_0072c8e0 != -1)
          {
            ...
            if (raw_mana_available[player][DAT_0072c8e0] > 0)
            {
              ...
              if (s.produced_special_colors != 0)
              {
                ...
                FUN_00435b18(
                    mana_charge, s.chosen_color, s.amount_to_take, &x_value, max_x_value, player, DAT_0072c8e0, s.mana_paid_by_color, &s.mana_paid_total);
                copy_mana_pool_to_display();
              }
            }
          }
        ...
        else
        {
                ...
                (((s.instance->color & 3) == 0) || (s.card_data->type & TYPE_CREATURE) == 0))))
          {
            ...
            if ((s.instance->state & 0x800002) == STATE_IN_PLAY)
            {
              ...
              {
                push_card_onto_stack(player, s.selected_card, EVENT_ACTIVATE, player, 0);
...
int FUN_0043479e(char *prompt, int *mana_cost, int x_paid, int max_x)
{
  ...
  if (s.action != 0xffffffffU)
  {
    ...
    s.stack_player = DAT_009251d8[stack_size].x;
    s.stack_card = DAT_009251d8[stack_size].y;
...
void FUN_0043410a(int player, int *mana_paid_by_color, int *total_mana_paid, int max_colorless)
{
  ...
  for (color = 0; color < 7; ++color)
  {
    ...
    {
      FUN_00435b18(mana_charge, color, 1, (int *)0, 0, player, color, mana_paid_by_color, total_mana_paid);
    }
  }

  while (mana_charge[6] > 0 && raw_mana_available[player][0] > 0 && max_colorless < mana_charge[6])
  {
    FUN_00435b18(mana_charge, 6, 1, (int *)0, 0, player, 0, mana_paid_by_color, total_mana_paid);
  }
  ...
  if (mana_charge[COLOR_ARTIFACT] != 0)
  {
    for (color = 0; color < 7; ++color)
    {
      if (mana_charge[COLOR_ARTIFACT] > 0)
      {
        ...
        {
          FUN_00435b18(mana_charge, COLOR_ARTIFACT, 1, (int *)0, 0, player, color, mana_paid_by_color, total_mana_paid);
        }
      }
      else
      {
                 ...
                 int include_special_mana)
{
  ...
  {
    result = FUN_004346ab(mana_charge, x_value, max_x_value);
    if (result == 0)
    {
        ...
        if (result != 0 && dispatch_event_to_single_card(player, s.current_card, EVENT_CAN_ACTIVATE, 1 - player, -1) != 0)
        {
          ...
          if (include_special_mana != 0 && !s.can_activate)
          {
            ...
            while (s.dual_mana_slot < 10 && unk_007161e0[player][s.dual_mana_slot] != -1)
            {
              ...
              if (mana_charge[unk_007161e0[player][s.dual_mana_slot] >> 16] > 0 && (((unsigned int)(1 << s.produced_color) & (unsigned int)(unsigned char)PLAYER_CARD_INSTANCE(player, s.current_card).mana_color) != 0))
              {
                s.can_activate = 1;
              }
              else if (mana_charge[unk_007161e0[player][s.dual_mana_slot] >> 16] == -1 && x_value < max_x_value && max_x_value != -1 && (((unsigned int)(1 << s.produced_color) & (unsigned int)(unsigned char)PLAYER_CARD_INSTANCE(player, s.current_card).mana_color) != 0))
              {
                s.can_activate = 1;
              }
              ++s.dual_mana_slot;
        ...
        if (result != 0 && dispatch_event_to_single_card(player, s.current_card, EVENT_CAN_ACTIVATE, 1 - player, -1) != 0)
        {
          ...
          if (include_special_mana != 0 && !s.any_variable_costs)
          {
            ...
            while (s.dual_mana_slot < 10 && unk_007161e0[player][s.dual_mana_slot] != -1)
            {
              ...
              if (max_x_value == -1 && mana_charge[unk_007161e0[player][s.dual_mana_slot] >> 16] == -1 && (((unsigned int)(1 << s.special_color) & (unsigned int)(unsigned char)PLAYER_CARD_INSTANCE(player, s.current_card).mana_color) != 0))
              {
                s.any_variable_costs = 1;
              }
              ++s.dual_mana_slot;
                           ...
                           int dialog_mode)
{
  struct
  {
    ...
    int action;                       /* ebp - 0x100 (FUN_00443898() result) */
  ...
  if (spell_fizzled == 1 || (g_duel_ai_mode_state == 1 && nonactive_player == player))
  {
    return -1;
  }
  ...
  if ((player == active_player && (g_duel_network_flags & 2) == 0) || g_duel_ai_mode_state == 1 || g_duel_network_state != 0)
  {
    ...
    for (s.current_player = 0; s.current_player < 2; s.current_player++)
    {
      ...
      for (s.current_card = 0; s.current_card < active_cards_count[s.current_player]; s.current_card++)
      {
        ...
        if (((PLAYER_CARD_INSTANCE(s.current_player, s.current_card).state & 0x800002) != 2) &&
            !(nonactive_player == player && g_duel_network_state != 0))
        {
          continue;
        }
    ...
    select_card_for_action_retry:
      ...
      if (g_duel_network_state != 0)
      {
        ...
        if (nonactive_player != unk_00742fcc)
        {
    ...
    if (g_duel_ai_mode_state == 1)
    {
      unk_00939340 = internal_rand(s.candidate_count);
      unk_00925bb8 = ((((s.candidate_players[unk_00939340] == 0) - 1) & 0x100) |
                      (s.candidate_cards[unk_00939340] & 0xff) |
                      0x4000);
      FUN_004e4f11();
    }
    else
    ...
    unk_00742fcc = s.candidate_players[unk_00939340];
    return s.candidate_cards[unk_00939340];
  }
...
int FUN_004e4ff3(int a1)
{
  if (g_duel_ai_mode_state != 1)
  {
    unk_00925bb8 = unk_006a1db8[a1 + recorded_action_count];
    if (unk_00925bb8 != -1)
    {
      unk_00925bb8 &= 0xfff;
    }
...
int C_get_abilities(int player, int card, event_t event, int new_attacking_card)
{
  ...
  result = event_result;
  if (is_in_play(player, card) && event == EVENT_TOUGHNESS && (global_cards_data[unk_008b4dd0].type & TYPE_CREATURE) != 0 && ((int)result < 1 || (int)result <= (int)(short)instance->damage_on_card) && ((*((char *)instance + 0x1b) & 4) == 0) && trigger_condition == -1 && (land_can_be_played & 0x204) == 0)
  {
                                    ...
                                    target_state_t illegal_state)
{
  ...
  if (tgt_card == -1)
  {
    ...
    {
      if ((((nonactive_player == who_chooses) || ((g_duel_network_flags & 2) != 0)) && ((allowed_controller & ANYBODY) != 0)) || (who_chooses == AI && ((preferred_controller & ANYBODY) != 0)))
      {
        s.tmp_d8 = AI;
        s.tmp_dc = 1;
      }
      else if ((((nonactive_player == who_chooses) || ((g_duel_network_flags & 2) != 0)) && ((allowed_controller & HUMAN) != 0)) || (who_chooses == AI && ((preferred_controller & HUMAN) != 0)))
      {
        s.tmp_d8 = HUMAN;
        s.tmp_dc = 1;
      }
...
int resolve_top_card_on_stack(void)
{
  ...
  {
    process_damage_prevention(human_player);
    regenerate_or_graveyard_triggers();
  }
...
void process_damage_prevention(int player)
{
  ...
  while (1)
  {
    if (((g_duel_network_flags & 2) == 0) && g_duel_ai_mode_state != 1 && DAT_0072c8e4 == 0)
    {
...
int should_skip_phase(int player)
{
  if ((g_duel_network_flags & 2) != 0)
  {
    ...
    if (previous_stop_phase == current_phase && human_player == previous_stop_phase_player)
    {
      return 0;
    }

    if (stop_phase == current_phase && human_player == stop_phase_player)
    {
      return 0;
    }
  ...
  if (human_player != stop_phase_player)
  {
    return 1;
  }
...
int FUN_0044541f(int param_1)
{
  ...
  if (param_1 != 0)
  {
    ...
    if (human_player == nonactive_player)
    {
      if (human_player == stop_phase_player && current_phase == stop_phase)
      {
        can_respond = 1;
      }
    }
    else if (human_player == previous_stop_phase_player && previous_stop_phase == current_phase)
    {
      can_respond = 1;
    }
    if (should_skip_phase(human_player) == 0 && can_respond != 0)
    {
      return 1;
    }
    return 0;
  ...
  if (should_skip_phase(1 - human_player) == 0 && can_respond != 0)
  {
    return 1;
  }
  return 0;
...
unsigned int FUN_00447f80(void)
{
  ...
  unsigned int diff = ticks - unk_0093f4b4 - g_duel_tick_adjustment;
