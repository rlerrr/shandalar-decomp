...
int card_power_up(int player, int card, event_t event)
{
  if (PLAYER_CARD_INSTANCE(player, card).damage_target_card == g_affected_card)
  {
    if ((int)PLAYER_CARD_INSTANCE(player, card).damage_target_player == g_affected_card_controller)
    {
      ...
      {
        if ((PLAYER_CARD_INSTANCE(player, card).info_slot & 0x80000) != 0 &&
            PLAYER_CARD_INSTANCE(player, card).damage_source_card != -1)
        {
          PLAYER_CARD_INSTANCE(player, card).counter_power =
              (short)(DAMAGE_SOURCE_CARD_INSTANCE(player, card).info_slot &
                      0xff);
          PLAYER_CARD_INSTANCE(player, card).counter_toughness =
              (short)((DAMAGE_SOURCE_CARD_INSTANCE(player, card).info_slot >>
                       8) &
                      0xff);
          if ((PLAYER_CARD_INSTANCE(player, card).counter_power & 0x80) != 0)
          {
            PLAYER_CARD_INSTANCE(player, card).counter_power |= 0xff00;
          }
          if ((PLAYER_CARD_INSTANCE(player, card).counter_toughness & 0x80) != 0)
          {
            PLAYER_CARD_INSTANCE(player, card).counter_toughness |= 0xff00;
          }
        }
        ...
        {
          g_event_result += PLAYER_CARD_INSTANCE(player, card).counter_toughness;
        }
      }
    }
  }

  if ((PLAYER_CARD_INSTANCE(player, card).token_status & STATUS_PERMANENT) == 0 &&
...
{
  if ((event == EVENT_CLEANUP || event == EVENT_SHOULD_AI_PLAY) &&
      (int)PLAYER_CARD_INSTANCE(player, card).damage_target_player == g_current_player)
  {
    if (PLAYER_CARD_INSTANCE(player, card).info_slot != 0)
    {
      DAMAGE_TARGET_CARD_INSTANCE(player, card).token_status &= ~STATUS_CANT_ATTACK;
      kill_card(player, card, KILL_BURY);
    }
    else
    {
...
int card_add_ability(int player, int card, event_t event)
{
  ...
  if (event == EVENT_GRAVEYARD_FROM_PLAY &&
      ...
      (int)PLAYER_CARD_INSTANCE(player, card).damage_target_player == g_affected_card_controller &&
      g_affected_card != -1 &&
      DAMAGE_TARGET_CARD_INSTANCE(player, card).kill_code != KILL_REMOVE &&
      DAMAGE_SOURCE_CARD_INSTANCE(player, card).internal_card_id != -1 &&
      (global_cards_data[DAMAGE_SOURCE_CARD_INSTANCE(player, card).internal_card_id]
               .id ==
           CARD_ID_FLYING_CARPET ||
       global_cards_data[DAMAGE_SOURCE_CARD_INSTANCE(player, card).internal_card_id]
               .code_pointer ==
           card_sandals_of_abdallah))
  {
  ...
  {
    if (PLAYER_CARD_INSTANCE(player, card).kill_code == 5)
    {
      if ((g_trigger_condition == TRIGGER_EOT || event == EVENT_SHOULD_AI_PLAY) &&
          (int)PLAYER_CARD_INSTANCE(player, card).damage_target_player == g_current_turn)
      {
    ...
    else if ((PLAYER_CARD_INSTANCE(player, card).token_status & STATUS_PERMANENT) == 0 &&
             (event == EVENT_CLEANUP || event == EVENT_SHOULD_AI_PLAY))
    {
      if ((int)PLAYER_CARD_INSTANCE(player, card).damage_target_player != -1)
      {
        DAMAGE_TARGET_CARD_INSTANCE(player, card).regen_status = 0x8000000;
      }
      kill_card(player, card, KILL_BURY);
    }
  }
...
int card_damage(int player, int card, event_t event)
{
  ...
  {
    s.target_abilities =
        C_get_abilities((int)(char)PLAYER_CARD_INSTANCE(player, card).damage_target_player, PLAYER_CARD_INSTANCE(player, card).damage_target_card,
                        EVENT_ABILITIES,
                        -1);
    if ((s.target_abilities & 0x1ff800) != 0)
    {
      s.source_color =
          single_color_test_bit_to_color_t(
              (color_test_t)DAMAGE_SOURCE_CARD_INSTANCE(player, card).color);
      if ((s.target_abilities & (0x800 << (s.source_color - 1))) != 0 ||
          ((s.target_abilities & 0x10000) != 0 &&
           (global_cards_data
                [s.protection_source_internal_card_id =
                     DAMAGE_SOURCE_CARD_INSTANCE(player, card).internal_card_id != -1
                         ? DAMAGE_SOURCE_CARD_INSTANCE(player, card).internal_card_id
                         : DAMAGE_SOURCE_CARD_INSTANCE(player, card).original_internal_card_id]
                    .type &
            TYPE_ARTIFACT)))
      {
      ...
      (PLAYER_CARD_INSTANCE(player, card).state & STATE_TAPPED) == 0)
  {
    ...
    if (DAMAGE_SOURCE_CARD_INSTANCE(player, card).original_internal_card_id == g_damage_effect_internal_card_id &&
        PLAYER_CARD_INSTANCE(player, card).info_slot != 0)
    {
      DAMAGE_SOURCE_CARD_INSTANCE(player, card).eot_toughness = (unsigned int)(int)PLAYER_CARD_INSTANCE(player, card).damage_source_player;
      create_damage_effect_copy_for_target_player((int)(char)PLAYER_CARD_INSTANCE(player, card).damage_source_player, PLAYER_CARD_INSTANCE(player, card).damage_source_card);
    }
    ...
    if (PLAYER_CARD_INSTANCE(player, card).damage_target_card == -1)
    {
      ...
      if (DAMAGE_SOURCE_CARD_INSTANCE(player, card).internal_card_id != -1 &&
          global_cards_data[DAMAGE_SOURCE_CARD_INSTANCE(player, card).internal_card_id]
                  .id == g_legacy_display_internal_card_id)
      {
        s.redirected_source_player =
            (int)DAMAGE_SOURCE_CARD_INSTANCE(player, card).damage_source_player;
        s.redirected_source_card =
            DAMAGE_SOURCE_CARD_INSTANCE(player, card).damage_source_card;
    ...
    else if (is_in_play((int)(char)PLAYER_CARD_INSTANCE(player, card).damage_target_player, PLAYER_CARD_INSTANCE(player, card).damage_target_card))
    {
      DAMAGE_TARGET_CARD_INSTANCE(player, card).damage_on_card += PLAYER_CARD_INSTANCE(player, card).info_slot;
      if (g_duel_ai_mode_state != 1)
      {
        play_sound_effect(0x16);
        request_duel_display_refresh_if_human((int)(char)PLAYER_CARD_INSTANCE(player, card).damage_target_player, PLAYER_CARD_INSTANCE(player, card).damage_target_card,
                                              6,
                                              2);
      }
    }
...
int card_asp_sting(int player, int card, event_t event)
{
  ...
  if ((g_trigger_condition == TRIGGER_DRAW_PHASE || event == EVENT_SHOULD_AI_PLAY) &&
      g_affected_card == card &&
      player == g_affected_card_controller &&
      ...
      PLAYER_CARD_INSTANCE(player, card).info_slot == 0)
  {
    ...
    if (event == EVENT_RESOLVE_TRIGGER || event == EVENT_SHOULD_AI_PLAY)
    {
      load_text("prompts.txt", "NAFS_ASP");
...
int card_stoning(int player, int card, event_t event)
{
      ...
      player == g_affected_card_controller)
  {
    ...
    if (event == EVENT_RESOLVE_TRIGGER || event == EVENT_SHOULD_AI_PLAY)
    {
      if (PLAYER_CARD_INSTANCE(player, card).damage_target_player != -1)
      {
        kill_card((int)(char)PLAYER_CARD_INSTANCE(player, card).damage_target_player, PLAYER_CARD_INSTANCE(player, card).damage_target_card,
                  KILL_DESTROY);
      }
...
int card_t_elemental_FX(int player, int card, event_t event)
{
      ...
      player == g_affected_card_controller)
  {
    ...
    if (event == EVENT_RESOLVE_TRIGGER || event == EVENT_SHOULD_AI_PLAY)
    {
      ...
      if (is_in_play((int)(char)PLAYER_CARD_INSTANCE(player, card).damage_source_player, PLAYER_CARD_INSTANCE(player, card).damage_source_card) != 0)
      {
        kill_card((int)(char)PLAYER_CARD_INSTANCE(player, card).damage_source_player, PLAYER_CARD_INSTANCE(player, card).damage_source_card,
                  KILL_BURY);
      }
...
int card_marsh_gas_1(int player, int card, event_t event)
{
  if (event == EVENT_POWER &&
      ...
      (int)PLAYER_CARD_INSTANCE(player, card).damage_target_player == g_affected_card_controller &&
      g_affected_card != -1 &&
      is_in_play((int)(char)PLAYER_CARD_INSTANCE(player, card).damage_target_player, PLAYER_CARD_INSTANCE(player, card).damage_target_card) != 0 &&
  ...
  if (event == EVENT_TOUGHNESS &&
      ...
      (global_cards_data[DAMAGE_TARGET_CARD_INSTANCE(player, card).internal_card_id]
       ...
       TYPE_CREATURE) != 0)
  {
    g_event_result += PLAYER_CARD_INSTANCE(player, card).eot_toughness;
  }
...
int card_fog_effect(int player, int card, event_t event)
{
      ...
      (g_current_phase == PHASE_NORMAL_COMBAT_DAMAGE || g_current_phase == PHASE_FIRST_STRIKE_DAMAGE))
  {
    ...
    {
      source_iid = DAMAGE_SOURCE_CARD_INSTANCE(g_affected_card_controller, g_affected_card).original_internal_card_id;
    }
    else
    {
      source_iid = DAMAGE_SOURCE_CARD_INSTANCE(g_affected_card_controller, g_affected_card).internal_card_id;
    }

    if ((global_cards_data[source_iid].type & TYPE_CREATURE) != 0)
    {
...
int card_channel_1(int player, int card, event_t event)
{
  ...
  if (event == EVENT_CAN_ACTIVATE)
  {
    if (g_other_player == player && (g_duel_network_flags & 2) == 0 && g_life[player] == 1)
    {
      return 0;
    }
    return g_life[player] > 0;
  }

  if (event == EVENT_ACTIVATE)
  {
    done = 0;
    while (!done && g_spell_fizzled != 1)
    {
      load_text("prompts.txt", "CHANNEL");
      amount = choose_a_number(player, g_text_lines[0], 1);
      if (amount < 0)
      {
        g_spell_fizzled = 1;
      }
      else if (g_life[player] < amount)
      {
        if (g_duel_ai_mode_state != 1)
        {
          load_text("prompts.txt", "CHANNEL");
    ...
    {
      undeclare_mana_available_and_produce_it(player, COLOR_COLORLESS, amount);
  ...
  {
    declare_mana_available(player, COLOR_COLORLESS, g_life[player]);
  }
...
{
  if ((PLAYER_CARD_INSTANCE(player, card).token_status & STATUS_BERSERK) != 0)
  {
    if (PLAYER_CARD_INSTANCE(player, card).damage_target_card == g_affected_card &&
        (int)PLAYER_CARD_INSTANCE(player, card).damage_target_player == g_affected_card_controller &&
        g_affected_card != -1)
    ...
    if (card == g_affected_card &&
        ...
        (DAMAGE_TARGET_CARD_INSTANCE(player, card).state &
         0x40) != 0)
    {
      PLAYER_CARD_INSTANCE(player, card).kill_code = 5;
    }
  }

  if ((PLAYER_CARD_INSTANCE(player, card).token_status & STATUS_LEGACY_TYPECHANGE) != 0)
  {
    ...
    if (g_trigger_condition == TRIGGER_LEAVE_PLAY &&
        ...
        player == g_affected_card_controller &&
        player == g_current_turn &&
        (int)PLAYER_CARD_INSTANCE(player, card).damage_source_player == g_trigger_cause_controller &&
        PLAYER_CARD_INSTANCE(player, card).damage_source_card == g_trigger_cause)
    {
      if (event == EVENT_TRIGGER)
      {
        g_event_result |= 2;
      ...
      {
        PLAYER_CARD_INSTANCE(player, card).token_status &= ~STATUS_INVISIBLE_FX;
        kill_card(player, card, KILL_DESTROY);
      }
    }
  ...
  if ((PLAYER_CARD_INSTANCE(player, card).token_status & STATUS_ISLAND_SANCTUARY) != 0)
  {
    ...
    if (event == EVENT_ATTACK_LEGALITY &&
        (PLAYER_CARD_INSTANCE(g_affected_card_controller, g_affected_card).regen_status &
         PLAYER_CARD_INSTANCE(player, card).info_slot) == 0)
    {
      ++g_event_result;
    }
  ...
  if ((PLAYER_CARD_INSTANCE(player, card).token_status & STATUS_TIMEWALK) != 0)
  {
  ...
  if (card == g_affected_card && player == g_affected_card_controller)
  {
    if ((char)PLAYER_CARD_INSTANCE(player, card).kill_code == 5)
    {
      if ((g_trigger_condition == TRIGGER_EOT || event == EVENT_SHOULD_AI_PLAY) &&
          (int)PLAYER_CARD_INSTANCE(player, card).damage_target_player == g_current_turn)
      {
...
int card_asterisk_FX(int player, int card, event_t event)
{
  ...
  if (PLAYER_CARD_INSTANCE(player, card).damage_target_card == g_affected_card &&
      (int)PLAYER_CARD_INSTANCE(player, card).damage_target_player == g_affected_card_controller &&
      g_affected_card != -1 &&
      ((event == EVENT_POWER && (PLAYER_CARD_INSTANCE(player, card).eot_toughness & 4) != 0) ||
       (event == EVENT_TOUGHNESS && (PLAYER_CARD_INSTANCE(player, card).eot_toughness & 8) != 0)))
  {
    s.count = 0;
    s.mode = PLAYER_CARD_INSTANCE(player, card).eot_toughness & 0xf00;
    if (s.mode < 0x201)
    {
      if (s.mode == 0x200)
      {
        ...
        {
          if (((int)PLAYER_CARD_INSTANCE(player, card).damage_target_player == s.current_player &&
               (PLAYER_CARD_INSTANCE(player, card).eot_toughness & 1) != 0) ||
              ((int)PLAYER_CARD_INSTANCE(player, card).damage_target_player != s.current_player &&
               (PLAYER_CARD_INSTANCE(player, card).eot_toughness & 2) != 0))
          {
            for (s.current_card = 0; s.current_card < g_active_cards_count[s.current_player]; ++s.current_card)
            {
              s.current_iid = PLAYER_CARD_INSTANCE(s.current_player, s.current_card).internal_card_id;
              if (is_in_play(s.current_player, s.current_card) != 0)
              {
                if ((PLAYER_CARD_INSTANCE(player, card).eot_toughness & 0x100000) == 0)
                {
                  if (global_cards_data[PLAYER_CARD_INSTANCE(player, card).info_slot].id ==
                      global_cards_data[s.current_iid].id)
                  {
                    ++s.count;
                  }
                }
                else if ((PLAYER_CARD_INSTANCE(player, card).info_slot & global_cards_data[s.current_iid].type) != 0)
                {
                  ++s.count;
                }
              }
      ...
      else if (s.mode == 0x100)
      {
        if ((PLAYER_CARD_INSTANCE(player, card).eot_toughness & 1) != 0)
        {
          s.current_iid = get_hacked_color((int)(char)PLAYER_CARD_INSTANCE(player, card).damage_target_player, PLAYER_CARD_INSTANCE(player, card).damage_target_card,
                                           PLAYER_CARD_INSTANCE(player, card).info_slot);
          s.count = g_basiclandtypes_controlled[PLAYER_CARD_INSTANCE(player, card).damage_target_player][s.current_iid];
        }
        if ((PLAYER_CARD_INSTANCE(player, card).eot_toughness & 2) != 0)
        {
          s.current_iid = get_hacked_color((int)(char)PLAYER_CARD_INSTANCE(player, card).damage_target_player, PLAYER_CARD_INSTANCE(player, card).damage_target_card,
                                           PLAYER_CARD_INSTANCE(player, card).info_slot);
          s.count += g_basiclandtypes_controlled[1 - PLAYER_CARD_INSTANCE(player, card).damage_target_player][s.current_iid];
        }
      }
    }
    else if (s.mode == 0x400)
    {
      if (event == EVENT_POWER)
      {
        s.count = PLAYER_CARD_INSTANCE(player, card).info_slot & 0xff;
      }
      else
      {
        s.count = BYTE1(PLAYER_CARD_INSTANCE(player, card).info_slot);
      }
    }
    else if (s.mode == 0x800)
    {
      for (s.current_player = 0; s.current_player < 2; ++s.current_player)
      {
        if (((int)PLAYER_CARD_INSTANCE(player, card).damage_target_player == s.current_player &&
             ...
             (PLAYER_CARD_INSTANCE(player, card).eot_toughness & 2) != 0))
        {
          for (s.current_card = 0; s.current_card < g_active_cards_count[s.current_player]; ++s.current_card)
          {
            ...
            {
              s.current_iid = PLAYER_CARD_INSTANCE(s.current_player, s.current_card).internal_card_id;
              if ((global_cards_data[s.current_iid].type & TYPE_CREATURE) != 0 &&
                  global_cards_data[s.current_iid].subtype != 0)
              {
                ++s.count;
              }
            }
    ...
    g_event_result += s.count;
    if (event == EVENT_POWER)
    {
      PLAYER_CARD_INSTANCE(player, card).counter_power = (short)s.count;
    }
    else
    {
      PLAYER_CARD_INSTANCE(player, card).counter_toughness = (short)s.count;
    }
  }
...
int card_piggy_FX(int player, int card, event_t event)
{
  ...
  {
    s.toughness_delta = 0;
    s.power_delta = s.toughness_delta;
    ...
    for (; s.current_card < g_active_cards_count[s.target_player] && s.found == 0; ++s.current_card)
    {
      if (PLAYER_CARD_INSTANCE(s.target_player, s.current_card).internal_card_id == g_duel_generated_internal_card_id_1a &&
          ...
          PLAYER_CARD_INSTANCE(player, card).damage_target_card ==
              PLAYER_CARD_INSTANCE(s.target_player, s.current_card).damage_target_card)
      {
        s.found = 1;
        s.power_delta = -PLAYER_CARD_INSTANCE(s.target_player, s.current_card).counter_power;
        s.toughness_delta = -PLAYER_CARD_INSTANCE(s.target_player, s.current_card).counter_toughness;
      }
    }

    if (event == EVENT_POWER)
    {
      g_event_result += PLAYER_CARD_INSTANCE(player, card).counter_power + s.power_delta;
    }
    else
    {
      g_event_result += PLAYER_CARD_INSTANCE(player, card).counter_toughness + s.toughness_delta;
    }
  }
...
int titanias_leg_kill_invalid_animation(int source_player, int source_card, int test_player, int test_card, int internal_card_id)
{
  if (PLAYER_CARD_INSTANCE(source_player, source_card).damage_target_card ==
  ...
  {
    if ((global_cards_data[DAMAGE_TARGET_CARD_INSTANCE(test_player, test_card).internal_card_id]
             .type &
         TYPE_CREATURE) == 0)
    {
...
{
  if ((DAMAGE_TARGET_CARD_INSTANCE(player, card).token_status &
       STATUS_DYING) != 0)
  {
    DAMAGE_TARGET_CARD_INSTANCE(player, card).kill_code = KILL_REMOVE;
  }
...
// FUNCTION: SHANDALAR 0x004aea1d
int sirens_call_attack_rating_callback(int player, int card, int internal_card_id)
{
  if (g_current_player == player && (PLAYER_CARD_INSTANCE(player, card).state & STATE_UNKNOWN8000) == 0)
  {
...
int card_sirens_call_FX(int player, int card, event_t event)
{
  ...
  {
    dispatch_three_arg_callback_to_cards_in_play(sirens_call_attack_rating_callback, -1);
  }
    ...
    for (current_card = 0; current_card < g_active_cards_count[target_player]; ++current_card)
    {
      ...
      if (is_in_play(target_player, current_card) != 0)
      {
        if ((global_cards_data[current_iid].type & TYPE_CREATURE) != 0 &&
            ...
            (PLAYER_CARD_INSTANCE(target_player, current_card).state &
             (STATE_ATTACKED | STATE_SUMMONSICK_NOATTACK | STATE_SUMMONSICK_NOTAP)) == 0)
        {
...
int card_ebony_horse_FX(int player, int card, event_t event)
{
  if (event == EVENT_PREVENT_DAMAGE &&
      (g_current_phase == PHASE_NORMAL_COMBAT_DAMAGE || g_current_phase == PHASE_FIRST_STRIKE_DAMAGE))
  {
    if (PLAYER_CARD_INSTANCE(player, card).damage_target_player ==
...
int card_graveyard(int player, int card, event_t event)
{
      ...
      player == g_current_turn)
  {
    ...
    if (event == EVENT_RESOLVE_TRIGGER)
    {
      if (PLAYER_CARD_INSTANCE(player, card).original_internal_card_id == find_internal_card_id_by_csv_id(CARD_ID_ONULET))
      {
        ...
        gain_life(player, s.amount, player, card);
      }
      ...
      if (PLAYER_CARD_INSTANCE(player, card).original_internal_card_id == find_internal_card_id_by_csv_id(CARD_ID_VOLCANIC_ERUPTION))
      {
        for (s.current_player = 0; s.current_player < 2; ++s.current_player)
        {
          ...
          for (s.current_card = 0; s.current_card < g_active_cards_count[s.current_player]; ++s.current_card)
          {
            if (is_in_play(s.current_player, s.current_card) != 0 &&
                (global_cards_data[PLAYER_CARD_INSTANCE(s.current_player, s.current_card).internal_card_id].type &
                 TYPE_CREATURE) != 0)
            {
              damage_creature(s.current_player,
                              s.current_card,
                              PLAYER_CARD_INSTANCE(player, card).info_slot,
                              player,
                              card);
      ...
      if (PLAYER_CARD_INSTANCE(player, card).original_internal_card_id == find_internal_card_id_by_csv_id(CARD_ID_ABU_JAFAR))
      {
        s.target_player = 1 - player;
        if (PLAYER_CARD_INSTANCE(player, card).info_slot == -2)
        {
          for (s.desert_card = 0; s.desert_card < g_active_cards_count[s.target_player]; ++s.desert_card)
          {
            if (is_in_play(s.target_player, s.desert_card) != 0 &&
                PLAYER_CARD_INSTANCE(s.target_player, s.desert_card).blocking ==
                    PLAYER_CARD_INSTANCE(player, card).damage_source_card)
            {
              kill_card(s.target_player, s.desert_card, KILL_BURY);
            }
          }
        }
        else
        {
          s.blocked_group =
              (int)PLAYER_CARD_INSTANCE(s.target_player, PLAYER_CARD_INSTANCE(player, card).info_slot).blocking;
          if (s.blocked_group == -1)
          {
            kill_card(s.target_player, PLAYER_CARD_INSTANCE(player, card).info_slot, KILL_BURY);
          }
          else
          {
            for (s.desert_card = 0; s.desert_card < g_active_cards_count[s.target_player]; ++s.desert_card)
            {
              if (is_in_play(s.target_player, s.desert_card) != 0 &&
                  PLAYER_CARD_INSTANCE(s.target_player, s.desert_card).blocking == s.blocked_group)
              {
                kill_card(s.target_player, s.desert_card, KILL_BURY);
              }
            }
...
int card_damage_legacy(int player, int card, event_t event)
{
      ...
      player == g_current_turn)
  {
    ...
    if (event == EVENT_RESOLVE_TRIGGER)
    {
      if (PLAYER_CARD_INSTANCE(player, card).original_internal_card_id == find_internal_card_id_by_csv_id(CARD_ID_DRAIN_LIFE) ||
          PLAYER_CARD_INSTANCE(player, card).original_internal_card_id == find_internal_card_id_by_csv_id(CARD_ID_SYPHON_SOUL))
      {
        s.current_player = 0;
        s.total = 0;
        for (; s.current_player < 2; ++s.current_player)
        {
          for (s.current_card = 0; s.current_card < g_active_cards_count[s.current_player]; ++s.current_card)
          {
            if (PLAYER_CARD_INSTANCE(s.current_player, s.current_card).internal_card_id ==
                    g_damage_card_internal_card_id &&
                PLAYER_CARD_INSTANCE(player, card).damage_source_player ==
                    PLAYER_CARD_INSTANCE(s.current_player, s.current_card).damage_source_player &&
                PLAYER_CARD_INSTANCE(player, card).damage_source_card ==
                    PLAYER_CARD_INSTANCE(s.current_player, s.current_card).damage_source_card)
            {
              if (PLAYER_CARD_INSTANCE(s.current_player, s.current_card).damage_target_card == -1)
              {
                s.amount = PLAYER_CARD_INSTANCE(s.current_player, s.current_card).info_slot;
                if (g_life[(int)PLAYER_CARD_INSTANCE(s.current_player, s.current_card).damage_target_player] < 0)
                {
                  s.amount += g_life[(int)PLAYER_CARD_INSTANCE(s.current_player, s.current_card).damage_target_player];
                }
              }
              else
              {
                if (C_get_abilities((int)(char)PLAYER_CARD_INSTANCE(s.current_player, s.current_card).damage_target_player, PLAYER_CARD_INSTANCE(s.current_player, s.current_card).damage_target_card,
                                    EVENT_TOUGHNESS,
                                    -1) < PLAYER_CARD_INSTANCE(s.current_player, s.current_card).info_slot)
                {
                  s.amount = C_get_abilities(
                      (int)(char)PLAYER_CARD_INSTANCE(s.current_player, s.current_card).damage_target_player, PLAYER_CARD_INSTANCE(s.current_player, s.current_card).damage_target_card,
                      EVENT_TOUGHNESS,
                      -1);
                }
                else
        ...
        gain_life(player, s.total, player, card);
      }
      kill_card(player, card, KILL_REMOVE);
    }
  }
