...
int card_tempest_efreet(int player, int card, event_t event)
{
  ...
  if (event == EVENT_ACTIVATE)
  {
    ...
    PLAYER_CARD_INSTANCE(player, card).targets[0].card =
        do_dialog(PLAYER_CARD_INSTANCE(player, card).targets[0].player,
                  player,
                  card,
                  -1,
                  -1,
  ...
  if (event == EVENT_RESOLVE_ACTIVATION)
  {
    ...
    choice = PLAYER_CARD_INSTANCE(player, card).targets[0].card;
  ...
  if (event == EVENT_RESOLVE_ACTIVATION)
  {
    ...
    else
    {
      ...
      if (s.legacy_card != -1)
      {
        ...
        {
          PLAYER_CARD_INSTANCE(player, s.legacy_card).original_internal_card_id =
              animated_internal_card_id;
          global_cards_data[animated_internal_card_id].type = TYPE_ARTIFACT | TYPE_CREATURE;
...
int card_dragon_whelp(int player, int card, event_t event)
{
  ...
  else if (event == EVENT_ACTIVATE)
  {
    ...
    if (result != 0)
    {
      ...
      if (player == g_current_player)
      {
        if ((((player == g_other_player) && ((g_duel_network_flags & 2) == 0)) || ((PLAYER_CARD_INSTANCE(player, card).eot_toughness & 0xff0000) == 0x30000)) || g_recorded_action_player != 1)
        {
          g_max_x_value = -1;
        }
        else
...
int card_birds_of_paradise(int player, int card, event_t event)
{
  ...
  else if ((event == EVENT_ACTIVATE) && ((instance->state & STATE_TAPPED) == 0))
  {
    ...
    if ((((player == g_other_player) && ((g_duel_network_flags & 2) == 0)) || (g_duel_ai_mode_state == 1)) || (g_duel_network_state != 0))
    {
      ...
      {
        if (((g_required_mana_color_mask & (1 << (((unsigned char)current_color) & 0x1f))) != 0) && ((available_colors & (1 << (((unsigned char)current_color) & 0x1f))) != 0))
        {
          choice_hint = current_color;
        }
        ++current_color;
      }
      if ((choice_hint == -1) && ((g_required_mana_color_mask & 1) != 0))
      {
        choice_hint = COLOR_BLACK;
      }
      if ((choice_hint == -1) && ((g_required_mana_color_mask & 0x40) != 0))
      {
        choice_hint = COLOR_BLACK;
      }
      if (choice_hint == -1)
