...
char g_duel_main_phase_text_buffer[0x100];
...
{
  int card;
  for (card = 0; g_active_cards_count[player] > card;
       card++)
  {
...
int update_attacker_count_and_check_combat_done(int player)
{
  ...
  if (g_attacking_creature_count != 0 || legal_attacker_count != 0)
    return 0;
...
int player_has_available_blocker(int player)
{
  ...
  do
  {
    if (g_active_cards_count[s.attacking_player] <= s.attacker)
    {
      return 0;
    }
        ...
        ((global_card_instances[s.attacking_player][s.attacker].state & STATE_ATTACKING) != 0))
    {
      ...
      for (s.blocker = 0; s.blocker < g_active_cards_count[player]; s.blocker++)
      {
        if (((global_card_instances[s.attacking_player][s.attacker].internal_card_id != -1) &&
...
void mark_blocked_attackers(int player)
{
  ...
  for (s.attacker = 0; s.attacker < g_active_cards_count[player]; s.attacker++)
  {
    ...
    {
      s.is_blocked = 0;
      for (s.card = 0; s.card < g_active_cards_count[s.opponent]; s.card++)
      {
...
int human_has_phase_stop(phase_t phase)
{
  if ((char)g_duel_phase_stop_settings[g_current_player].phase_flags[phase] != 0)
  {
    return 1;
  }
  if ((g_stop_phase == phase) && (g_stop_phase_player == g_current_player))
  {
    return 1;
  }
  if ((g_previous_stop_phase == phase) && (g_previous_stop_phase_player == g_current_player))
  {
    return 1;
  }
  return 0;
...
restart_active_main_phase_ai_prompt:
  ...
  {
    TENTATIVE_reassess_all_cards(0, 0xff);
    start_ai_decision_search(1, ((g_current_phase < PHASE_MAIN2) - 1 & 0xffffffd3) + 0x5a);
    g_ai_search_target_depth = 0;
  }
  ...
  *phase_value = 0;
  ...
  do
  {
    ...
    if ((((player == g_other_player) && ((g_duel_network_flags & 2) == 0)) && (g_duel_ai_mode_state != 1)) &&
        ((g_duel_state_00925d30 & 2U) != 0))
    {
      g_duel_state_00925d30 = 0;
      goto restart_active_main_phase_ai_prompt;
    }
    g_adventure_duel_internal_card_id = 0;
    if ((g_duel_ai_mode_state != 1) && (g_duel_winner != '\0'))
    {
      show_opponent_taunt(&g_duel_winner);
    }
    if ((player == g_active_player) || ((g_duel_network_flags & 2) != 0))
    {
      ...
      do
      {
        ...
        if ((g_duel_network_flags & 2) != 0)
        {
          ...
          s.player_can_stop_here = player_can_stop_at_phase(player, g_current_phase);
        }
        if ((should_skip_phase(player) != 0 && (g_current_phase != PHASE_DECLARE_ATTACKERS || legal_attacker_count == 0)) ||
            (should_skip_phase(player) == 0 && g_current_phase == PHASE_MAIN2 && player_can_stop_at_phase(player, g_current_phase) == 0))
        {
        ...
        if (main_phase_selected_card == 0xfffffffe)
        {
          ...
          else
          {
            ...
            g_previous_stop_phase = g_previous_stop_phase_player;
          }
          main_phase_selected_card = 0xffffffff;
        }

        if ((g_current_phase == PHASE_DECLARE_ATTACKERS) && ((int)main_phase_selected_card < 0))
        {
      ...
      {
        if (g_current_phase < PHASE_AFTER_BLOCKING)
        {
          if ((g_stop_phase > PHASE_AFTER_BLOCKING) || (g_previous_stop_phase > PHASE_AFTER_BLOCKING))
          {
        ...
        if (g_current_phase < PHASE_BEFORE_BLOCKING)
        {
          if (g_stop_phase > PHASE_BEFORE_BLOCKING)
          {
            g_phase_stop_suppressed = 1;
          }
          else
            ...
            (g_previous_stop_phase != -1))))
      {
        if (g_current_phase <= PHASE_MAIN1)
        {
          ...
          if (!((g_attacking_creature_count == 0) &&
                ...
                ((g_active_player != g_stop_phase_player) || (g_stop_phase != PHASE_DECLARE_ATTACKERS)) &&
                ((g_other_player != g_previous_stop_phase_player) || (g_previous_stop_phase != PHASE_DECLARE_ATTACKERS)) &&
                (((char)g_duel_phase_stop_settings[player].phase_flags[PHASE_DECLARE_ATTACKERS] & (PHASE_STOP_ENABLED | PHASE_STOP_OPPONENT)) == 0)))
          {
            ...
            if ((g_duel_network_flags & 2) != 0)
            {
              ...
              s.saved_response_state = g_ai_decision_parameter;
              reset_trigger_dispatch_state();
              g_phase_response_window_open = 1;
              g_phase_stop_suppressed = 0;
              s.human_stop_enabled = 0;
              if (((g_active_player == g_current_player) &&
                   (((char)g_duel_phase_stop_settings[g_current_player].phase_flags[g_current_phase] & PHASE_STOP_OPPONENT) != 0)) ||
                  ((g_other_player == g_current_player &&
                    (((char)g_duel_phase_stop_settings[g_current_player].phase_flags[g_current_phase] & PHASE_STOP_ENABLED) != 0))))
              {
              ...
              if ((should_skip_phase(player) == 0) &&
                  (((s.human_stop_enabled != 0 ||
                     ((g_current_player == g_previous_stop_phase_player && (g_previous_stop_phase == g_current_phase)))) ||
                    ((g_current_player == g_stop_phase_player && (g_stop_phase == g_current_phase))))))
              {
              ...
              if (g_ai_decision_parameter != 0)
              {
                ...
                if (process_response_actions(1 - g_current_player,
                                             gs_prompt_main_phase_precombat_cast_spells_008cc710) != 0)
                {
                  g_ai_decision_parameter = s.saved_response_state;
                  g_phase_response_window_open = 0;
                  g_phase_stop_suppressed = 0;
                  goto resume_main_phase_response_window;
                }
            ...
            if (((g_active_player == g_current_player) && (g_active_player == g_stop_phase_player)) &&
                (g_stop_phase == PHASE_DECLARE_ATTACKERS))
            {
              g_duel_phase_stop_settings[g_active_player].phase_flags[PHASE_DECLARE_ATTACKERS] = (char)g_duel_phase_stop_settings[g_active_player].phase_flags[PHASE_DECLARE_ATTACKERS] | PHASE_STOP_SUPPRESSED;
              g_duel_phase_stop_settings[g_active_player].phase_flags[PHASE_BEFORE_BLOCKING] = (char)g_duel_phase_stop_settings[g_active_player].phase_flags[PHASE_BEFORE_BLOCKING] | PHASE_STOP_SUPPRESSED;
              g_duel_phase_stop_settings[g_active_player].phase_flags[PHASE_AFTER_BLOCKING] = (char)g_duel_phase_stop_settings[g_active_player].phase_flags[PHASE_AFTER_BLOCKING] | PHASE_STOP_SUPPRESSED;
            }
            if ((((g_duel_network_flags & 2) != 0) && (g_other_player == g_current_player)) &&
                ((g_other_player == g_previous_stop_phase_player && (g_previous_stop_phase == PHASE_DECLARE_ATTACKERS))))
            {
              g_duel_phase_stop_settings[g_other_player].phase_flags[PHASE_DECLARE_ATTACKERS] = (char)g_duel_phase_stop_settings[g_other_player].phase_flags[PHASE_DECLARE_ATTACKERS] | PHASE_STOP_SUPPRESSED;
              g_duel_phase_stop_settings[g_other_player].phase_flags[PHASE_BEFORE_BLOCKING] = (char)g_duel_phase_stop_settings[g_other_player].phase_flags[PHASE_BEFORE_BLOCKING] | PHASE_STOP_SUPPRESSED;
              g_duel_phase_stop_settings[g_other_player].phase_flags[PHASE_AFTER_BLOCKING] = (char)g_duel_phase_stop_settings[g_other_player].phase_flags[PHASE_AFTER_BLOCKING] | PHASE_STOP_SUPPRESSED;
            }
            if ((should_skip_phase(player) != 0) &&
                (g_attacking_creature_count == 0) &&
        ...
        else
        {
          ...
          {
            strcpy(g_ui_message_buffer, "");
            if ((0 < g_attacking_creature_count) || (update_attacker_count_and_check_combat_done(player) == 0))
            {
              *phase_value = 1;
            }
          }
    ...
    else
    {
      ...
      if (main_phase_selected_card == 0xffffffff)
      {
        if (g_duel_ai_mode_state != 1)
        {
          allow_response(0, g_current_phase, gs_main_phase_008cee80, g_current_phase);
        }
        if (g_current_phase == PHASE_MAIN1)
        {
          g_current_phase = PHASE_DECLARE_ATTACKERS;
    ...
    g_adventure_duel_internal_card_id = 3;
    ...
    if (main_phase_selected_card != 0xffffffff)
    {
      ...
      if ((global_card_instances[player][main_phase_selected_card].state & 0x12) == 0)
      {
        ...
        if (g_current_phase == PHASE_DECLARE_ATTACKERS)
        {
          goto check_main_phase_abort;
        }
        ...
        {
          if (((player == g_active_player) && ((g_duel_network_flags & 2) == 0)) &&
              ((g_current_phase != PHASE_DECLARE_ATTACKERS && ((global_cards_data[main_phase_selected_internal_card_id].type & 1) == 0))))
          {
        ...
        resume_cast_selected_card:
          ...
          if ((((player == g_other_player) && ((g_duel_network_flags & 2) == 0)) &&
            ...
            goto restart_active_main_phase_ai_prompt;
          if ((g_duel_ai_mode_state == 1) && (g_ai_decision_code == 4))
          {
            *phase_value = 1;
          }
        }
      }
      else if (((g_current_phase <= PHASE_MAIN1) || (g_current_phase >= PHASE_MAIN2)) ||
               ((g_other_player == player && ((g_duel_network_flags & 2) == 0))))
      {
            ...
            (dispatch_event_to_single_card(player, main_phase_selected_card, 0x73, s.other_player, -1) != 0))
        {
          ...
          resume_activate_selected_card:
            ...
            if ((g_duel_ai_mode_state == 1) && (g_ai_decision_code == 7))
            {
              *phase_value = 1;
            }
          }
               ...
               (can_attack(player, main_phase_selected_card) != 0))
      {
        ...
        {
          push_affected_card_stack();
          strcpy(g_duel_main_phase_text_buffer, g_ui_message_buffer);
          g_trigger_cause_controller = player;
          g_trigger_cause = main_phase_selected_card;
          dispatch_trigger(player, 0xdc, gs_pay_for_attacker_007a7880, 1);
          strcpy(g_ui_message_buffer, g_duel_main_phase_text_buffer);
          pop_affected_card_stack();
        }
        if (g_combat_assignment_cancelled == 0)
        {
          ...
          if ((((C_get_abilities(player, main_phase_selected_card, 0x34, -1) & 0x200040) != 0) && (1 < g_attacking_creature_count)) && (g_recorded_action_player == 0))
          {
          ...
          if ((g_battlefield_extra_ability_flags & 0x80000) != 0)
          {
            g_trigger_cause_controller = player;
            g_trigger_cause = main_phase_selected_card;
            dispatch_trigger_twice_once_with_each_player_as_reason(g_current_player, 0xde, gs_attacker_selected_008b32d0, 0);
          }
  ...
  g_adventure_duel_internal_card_id = 4;
  g_response_processing_flags = 1;
  process_damage_prevention(player);
  if ((((player == g_other_player) && ((g_duel_network_flags & 2) == 0)) &&
       ((g_duel_ai_mode_state != 1 || (g_ai_decision_code != 1)))) &&
      (g_current_phase == PHASE_MAIN2))
  {
    goto finish_main_phase;
  }

  if (((player == g_other_player) && ((g_duel_network_flags & 2) == 0)) && (g_current_phase < PHASE_MAIN2))
  {
    ...
    {
      if (g_other_player == g_current_player)
      {
        g_stop_phase = -1;
      ...
      g_trigger_cause = 0;
      g_trigger_cause_controller = player;
      for (; g_trigger_cause < g_active_cards_count[player];
           g_trigger_cause = g_trigger_cause + 1)
      {
            ...
            ((global_card_instances[g_trigger_cause_controller][g_trigger_cause].state & 0x800006) == 6))
        {
          ...
          if (g_combat_assignment_cancelled != 0)
          {
            *(unsigned int *)&global_card_instances[g_trigger_cause_controller][g_trigger_cause].state &= 0xfffffffb;
            if ((char)global_card_instances[g_trigger_cause_controller][g_trigger_cause].blocking != -1)
            {
              s.bandmate_index = 0;
              s.bandmate_count = 0;
              for (; s.bandmate_index < g_active_cards_count[player]; s.bandmate_index = s.bandmate_index + 1)
              {
                if ((s.bandmate_index != g_trigger_cause) &&
                    ((char)global_card_instances[player][s.bandmate_index].blocking ==
                     (char)global_card_instances[g_trigger_cause_controller][g_trigger_cause].blocking))
                {
      ...
      resume_ai_attackers_response:
        ...
        g_phase_response_window_open = 1;
        s.allow_response_result = allow_response(-2, g_current_phase, gs_assign_attackers_008b45f0, PHASE_BEFORE_BLOCKING);
        g_phase_response_window_open = 0;
  ...
  if (((player == g_active_player) || ((g_duel_network_flags & 2) != 0)) && (g_current_phase < PHASE_MAIN2))
  {
    if ((should_skip_phase(player) != 0) &&
        (g_attacking_creature_count == 0) &&
...
resolve_combat_if_needed:
      ...
      (((player == g_active_player) || ((g_duel_network_flags & 2) != 0)) && (g_duel_ai_mode_state == 1)))
  {
    ...
    resume_after_blockers_response:
      ...
      g_current_phase = PHASE_AFTER_BLOCKING;
      update_phase_display(player, g_current_phase);
      if (TRACE_ENABLED)
      {
      ...
      g_phase_response_window_open = 1;
      s.allow_response_result = allow_response(-2, g_current_phase, gs_assign_blockers_008a8c40, PHASE_AFTER_BLOCKING);
      g_phase_response_window_open = 0;
    } while (s.allow_response_result != 0);
    if (TRACE_ENABLED)
    {
...
advance_to_postcombat_main:
  if (((player == g_active_player) || ((g_duel_network_flags & 2) != 0)) && (g_duel_ai_mode_state != 1))
  {
    if (g_current_phase < PHASE_MAIN2)
    {
      if (g_life[1] <= g_duel_summary.life_totals[1] / 2)
      {
  ...
  if (((player == g_other_player) && ((g_duel_network_flags & 2) == 0)) && (g_current_phase != PHASE_MAIN2))
  {
    ...
    if (g_duel_ai_mode_state == 1)
    {
      if ((g_ai_decision_code == 1) || (g_ai_decision_code == 2))
      {
...
finish_main_phase:
  g_adventure_duel_internal_card_id = 5;
  if (g_battlefield_extra_ability_flags & 2)
  {
